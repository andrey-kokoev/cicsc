...
export class WorkflowWorker {
  ...
  async pollAndExecute (tenant_id: string, params: { now: number }) {
    const { now } = params
    const instances = await this.store.list_due_workflow_instances({ tenant_id, now })
    let processed = 0
    let errors = 0
  ...
  private async processInstance (inst: WorkflowInstance, now: number) {
    ...
    if (!wf) {
      await this.store.update_workflow_instance({
    ...
    if (status === "waiting") {
      ...
      if (step && step.kind === "wait") {
        // Timeout reached
        currentStepName = step.on_timeout || step.next
        status = "running"
        await this.store.append_workflow_log({
          tenant_id: inst.tenant_id,
          workflow_id: inst.workflow_id,
          step_name: inst.current_step,
          action: "timeout",
          payload_json: "{}",
          ts: now,
        })
    ...
    while (status === "running") {
      ...
      if (result.action === "wait") {
        ...
        await this.store.update_workflow_instance({
          ...
          workflow_id: inst.workflow_id,
          status,
          wait_event_type: result.wait_event_type || null,
    ...
    if (status === "compensating") {
      await this.executeCompensation(inst, now)
      status = "compensated"
    }

    // Final update if state changed to completed/failed/compensated
    if (status !== "running") {
      await this.store.update_workflow_instance({
        tenant_id: inst.tenant_id,
        workflow_id: inst.workflow_id,
  ...
  private async executeCompensation (inst: WorkflowInstance, now: number) {
    const history = inst.history_json ? JSON.parse(inst.history_json) : []
    const wf = this.ir.workflows![inst.workflow_name]

    const successfulSteps = history.filter((h: any) => h.action === "next").reverse()
    const context = inst.context_json ? JSON.parse(inst.context_json) : {}

    for (const h of successfulSteps) {
      const step = wf.steps[h.step]
      if (step && step.kind === "command" && step.compensate) {
        const comp = step.compensate
        const env: VmEnv = {
          now,
          actor: `workflow:${inst.workflow_id}:compensate`,
          state: "workflow_compensating",
          input: {},
          attrs: {},
          row: {},
          arg: context,
        ...
        const input: any = {}
        for (const [k, ex] of Object.entries(comp.input_map)) {
          input[k] = evalExpr(ex, env)
        }
        try {
          const entity_id = input.entity_id || inst.workflow_id
          await this.runCommand(inst.tenant_id, comp.entity_type, entity_id, comp.command, input)
          await this.store.append_workflow_log({
            tenant_id: inst.tenant_id,
            workflow_id: inst.workflow_id,
            step_name: h.step,
          ...
          console.error(`Compensation failed for step ${h.step}: ${e.message}`)
          await this.store.append_workflow_log({
            tenant_id: inst.tenant_id,
            workflow_id: inst.workflow_id,
            step_name: h.step,
            action: "compensation_failed",
            payload_json: JSON.stringify({ error: e.message }),
            ts: now,
          })
        }
      }
  ...
  private async executeStep (
    ...
    context: any,
    now: number
  ): Promise<{ action: 'next' | 'wait' | 'end' | 'fail', next_step?: string, wait_until?: number, wait_event_type?: string, outcome?: any }> {
    switch (step.kind) {
      case 'end':
        return { action: 'end' }
      ...
      case 'wait': {
        if (step.timeout_seconds) {
          return { action: 'wait', wait_until: now + step.timeout_seconds, wait_event_type: step.event_type }
        }
        return { action: 'wait', wait_until: undefined, wait_event_type: step.event_type }
      }
