...
OUT_PATH="${1:-${ROOT_DIR}/docs/pilot/phase14-gate.json}"
cd "${ROOT_DIR}"
...
const phaseNo = 14
const checklistPath = 'docs/pilot/phase13-exit-checklist.json'
...
const reasons = []
if (!checklistPass) reasons.push('phase13_exit_checklist_not_pass')
if (!allChecked) reasons.push(`phase_${code}_series_incomplete`)

const report = {
  version: 'cicsc/phase14-gate-v1',
