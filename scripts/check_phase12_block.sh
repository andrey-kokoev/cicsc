...
OUT_PATH="${1:-${ROOT_DIR}/docs/pilot/phase12-gate.json}"
cd "${ROOT_DIR}"
...
const phaseNo = 12
const checklistPath = 'docs/pilot/phase11-exit-checklist.json'
...
const reasons = []
if (!checklistPass) reasons.push('phase11_exit_checklist_not_pass')
if (!allChecked) reasons.push(`roadmap_${code}_series_incomplete`)

const report = {
  version: 'cicsc/phase12-gate-v1',
