...
OUT_PATH="${1:-${ROOT_DIR}/docs/pilot/phase7-isolation-differential.json}"
...
run_check () {
  local name="$1"
  shift
  local log_path="${OUT_DIR}/phase7-isolation-differential.${name}.log"
  local status
...
CHECKS+=("$(run_check runtime_seq_isolation ./scripts/node_test.sh tests/runtime/execute-command-tx-seq.test.ts)")
...
{
  echo "{"
  echo "  \"version\": \"cicsc/phase7-isolation-differential-v1\","
...
echo "phase7 isolation differential report written: ${OUT_PATH}"
[[ "${overall}" == "pass" ]]
