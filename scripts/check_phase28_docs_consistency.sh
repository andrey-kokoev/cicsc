...
const phase = fs.readFileSync(path.resolve('PHASE28.md'), 'utf8')
...
const phaseMap = parse(phase, /^- \[(x| )\] P28\.(\d)\.(\d)\s+/gm, 'AS')
const roadmapMap = parse(roadmap, /^- \[(x| )\] AS(\d)\.(\d)\s+/gm, 'AS')
...
for (const [id] of roadmapMap.entries()) {
  if (!phaseMap.has(id)) mismatches.push({ id, reason: 'missing_in_phase28' })
}
