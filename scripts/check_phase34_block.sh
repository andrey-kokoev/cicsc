...
OUT_PATH="${1:-${ROOT_DIR}/docs/pilot/phase34-gate.json}"
cd "${ROOT_DIR}"
...
const phaseNo = 34
const checklistPath = 'docs/pilot/phase33-exit-checklist.json'
...
const rows = (executionStatus.rows ?? []).filter((r) => Number(r.phase_number) === 33)
...
const reasons = []
if (!checklistPass) reasons.push('phase33_exit_checklist_not_pass')
if (!allChecked) reasons.push(`phase_${code}_series_incomplete`)

const report = {
  version: 'cicsc/phase34-gate-v1',
