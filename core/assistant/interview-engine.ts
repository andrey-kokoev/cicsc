...
export class InterviewEngine {
    ...
    switch (this.state.currentStep) {
      ...
      case "CONFIRM":
        if (input.toLowerCase().includes("yes") || input.toLowerCase().includes("correct") || input.toLowerCase().includes("right")) {
          this.recomputeBlockingIssues()
          if (this.state.blockingIssues.length > 0) {
            return "Draft captured, but it is not deployable yet. Type '/spec' to review blocking issues."
          }
  ...
  getState (): InterviewState {
    ...
    return {
      ...
      entities: this.state.entities.map((e) => ({
        ...e,
        attrs: [...e.attrs],
        commands: e.commands.map((c) => ({ ...c, inputs: [...c.inputs] })),
      })),
      pendingClarifications: [...this.state.pendingClarifications],
  ...
  getSummary (): string {
    ...
    summary += `---\n`
    summary += `Deployable: ${this.state.blockingIssues.length === 0 ? "yes" : "no"}\n`
    if (this.state.blockingIssues.length > 0) {
      summary += `Blocking issues:\n`
      for (const issue of this.state.blockingIssues) {
        summary += `- ${issue}\n`
      }
    }
    summary += `\n`
  ...
  getStructuredSpec (): object {
    ...
    const spec = translator.translateToSpecJson(this.state)
    return {
      spec,
      deployable: this.state.blockingIssues.length === 0,
      blocking_issues: [...this.state.blockingIssues],
    }
  }

  private recomputeBlockingIssues (): void {
    const issues: string[] = []
    ...
    this.state.blockingIssues = issues
  }
}
