...
function makeSerializedAdapter (initial: MemState) {
...
describe("runtime seq allocation under concurrency", () => {
  it("allocates unique ordered seq values for concurrent commands on same stream", async () => {
    const init: MemState = {
      events: [],
      snapshots: new Map([
        [
    ...
    const mocked = makeSerializedAdapter(init)
    ...
    const seqs = [r1.seq_from, r2.seq_from].sort((a, b) => a - b)
    assert.deepEqual(seqs, [1, 2])

    const committed = mocked.getCommitted()
    const streamSeqs = committed.events
      .filter((e) => e.tenant_id === "t" && e.entity_type === "Ticket" && e.entity_id === "e1")
      .map((e) => e.seq)
      .sort((a, b) => a - b)
    assert.deepEqual(streamSeqs, [1, 2])
  })
})
