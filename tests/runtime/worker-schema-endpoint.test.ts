...
describe("worker /schema endpoint", () => {
  it("returns generated schema SQL for active tenant version", async () => {
    ...
    await bindTenant(fake.db as any, { tenant_id: "t", bundle_hash: put.bundle_hash, active_version: 3 })

    const res = await workerDefault.fetch(
      new Request("http://localhost/schema?tenant_id=t"),
      { DB: fake.db as any } as any
    )
    assert.equal(res.status, 200)
    const body = await res.json() as any
    assert.equal(body.ok, true)
    assert.equal(body.active_version, 3)
    assert.match(String(body.schema_sql), /CREATE TABLE IF NOT EXISTS events_v3/)
    assert.match(String(body.schema_sql), /CREATE TABLE IF NOT EXISTS snapshots_v3/)
  })
})
