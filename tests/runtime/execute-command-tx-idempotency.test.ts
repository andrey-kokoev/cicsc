...
function makeStore (backing: { committed: MemState }) {
  const adapter = {
    ...
    async tx<T> (fn: (tx: { exec: (sql: string, binds?: any[]) => Promise<{ rows?: any[] }> }) => Promise<T>): Promise<T> {
      const working = cloneState(backing.committed)
      const tx = {
...
describe("command idempotency across restart", () => {
  it("returns stored receipt and avoids duplicate events for repeated command_id", async () => {
    const backing = {
      committed: {
        events: [],
        snapshots: new Map([
    ...
    const store1 = makeStore(backing) // pre-restart process
    ...
    const store2 = makeStore(backing) // simulated restart: new adapter, same persisted state
