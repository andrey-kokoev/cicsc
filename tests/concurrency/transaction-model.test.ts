...
class TransactionExecutor {
  ...
  /** Test helper: pre-acquire lock to model overlapping transactions. */
  holdLockForTest(sid: StreamId, txId: string): void {
    this.locks.set(this.streamKey(sid), txId)
  }

  /** Test helper: release lock acquired via holdLockForTest. */
  releaseLockForTest(sid: StreamId): void {
    this.locks.delete(this.streamKey(sid))
  }
}
...
describe("P4.3.3: Write-write conflict tests and expected abort outcomes", () => {
  it("detects write-write conflict on same stream", () => {
    ...
    // Simulate tx1 currently holding the stream lock.
    executor.holdLockForTest(stream, "tx1")

    // tx2 attempts write on same stream and should abort.
    const tx2: Transaction = {
      id: "tx2",
      commands: [
        { stream, eventType: "Update", payload: {} },
      ]
    }
    
    const result2 = executor.execute(tx2, 1001)
    assert.strictEqual(result2.status, "aborted")
    if (result2.status === "aborted") {
      assert.match(result2.reason, /write-write conflict/)
    }
    assert.strictEqual(executor.getStreamSeq(stream), 0)

    executor.releaseLockForTest(stream)
  })
