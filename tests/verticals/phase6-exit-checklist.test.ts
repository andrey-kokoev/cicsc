...
describe("phase6 exit checklist", () => {
  it("is objective and artifact-mapped", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase6-exit-checklist.json")
    const checklist = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(checklist.version, "cicsc/phase6-exit-checklist-v1")
