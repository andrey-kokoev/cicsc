...
describe("phase15 deferred-surface closure report", () => {
  it("publishes residual policy with no unresolved criticals", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase15-deferred-surface-closure-report.json")
    const r = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(r.version, "cicsc/phase15-deferred-surface-closure-report-v1")
