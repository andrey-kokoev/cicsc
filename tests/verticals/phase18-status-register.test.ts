...
describe("phase18 status register", () => {
  it("tracks phase18 items with status and owner/date accountability", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase18-status-register.json")
    const doc = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(doc.version, "cicsc/phase18-status-register-v1")
