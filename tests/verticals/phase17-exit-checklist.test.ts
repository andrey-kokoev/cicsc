...
describe("phase17 exit checklist", () => {
  it("maps all exit items to artifacts with pass status", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase17-exit-checklist.json")
    const r = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(r.version, "cicsc/phase17-exit-checklist-v1")
