...
describe("phase16 exit checklist", () => {
  it("maps all exit items to artifacts with pass status", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase16-exit-checklist.json")
    const r = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(r.version, "cicsc/phase16-exit-checklist-v1")
