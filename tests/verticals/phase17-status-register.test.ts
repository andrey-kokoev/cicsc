...
describe("phase17 status register", () => {
  it("tracks phase17 items with status and owner/date accountability", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase17-status-register.json")
    const doc = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(doc.version, "cicsc/phase17-status-register-v1")
