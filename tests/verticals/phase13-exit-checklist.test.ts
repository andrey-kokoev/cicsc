...
describe("phase13 exit checklist", () => {
  it("is objective and mapped to concrete artifacts", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase13-exit-checklist.json")
    const checklist = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(checklist.version, "cicsc/phase13-exit-checklist-v1")
