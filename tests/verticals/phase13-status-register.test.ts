...
describe("phase13 status register", () => {
  it("tracks phase13 items with status and owner/date accountability", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase13-status-register.json")
    const doc = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(doc.version, "cicsc/phase13-status-register-v1")
