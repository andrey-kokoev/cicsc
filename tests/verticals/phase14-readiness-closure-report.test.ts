...
describe("phase14 readiness closure report", () => {
  it("publishes residual policy with no unresolved criticals", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase14-readiness-closure-report.json")
    const r = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(r.version, "cicsc/phase14-readiness-closure-report-v1")
