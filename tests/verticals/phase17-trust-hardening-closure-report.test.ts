...
describe("phase17 trust hardening closure report", () => {
  it("publishes residual policy with no unresolved criticals", () => {
    const p = path.resolve(process.cwd(), "docs/pilot/phase17-trust-hardening-closure-report.json")
    const r = JSON.parse(fs.readFileSync(p, "utf8"))

    assert.equal(r.version, "cicsc/phase17-trust-hardening-closure-report-v1")
